...
| Strategy Registry | `strategies/registry.yaml`, `strategies/STRATEGY_LEDGER.md` | READY (data-level); see `STRATEGY_REGISTRY_STATUS.md` |
...
human-readable report. Full contract: `FIVE_SKILL_ASSISTANT_RUNTIME_V1_SPEC.md`;
evidence: `FIVE_SKILL_ASSISTANT_RUNTIME_STATUS.md`. This is a **second**, separate
...
`ASSISTANT_RUNTIME_V1.md` for the architecture and `ASSISTANT_RUNTIME_V1_STATUS.md` for
...
skill deciding "sweep => short" on its own). See `ARCHITECTURE_CONFLICT_AUDIT.md` for the audit
...
`SESSION_TRADE_V1_SPEC.md` and `strategies/session_trade/contract.yaml`.

## Current registered strategies

See `strategies/registry.yaml` for the machine-readable list and `STRATEGY_REGISTRY_STATUS.md`
...
Skill-optimization pass, 2026-08-27. Full audit/evidence: `SKILL_OPTIMIZATION_STATUS.md`.
...
CORE (five-layer pyramid, primary trading intelligence)
  ...
  4. entry-confirmation      <- entry-confirmation-analysis skill, entry_confirmation/ package
                                 ...
                                 qualification UNSIGNED, see ENTRY_CONFIRMATION_V1_SPEC.md)
  5. trade-management        <- trade-management-analysis + risk-position-sizing skills,
                                 ...
                                 added 2026-08-28 -- see TRADE_MANAGEMENT_V1_SPEC.md)
...
|---|---|---|---|
| `trend-range-classification` | `ER_ONLY_V2` (`strategy_engine/session/classifier.py`) | `ST_ASIAN_SWEEP_5R_V1` | none -- `market-structure` reports structural direction, a different question (see `ARCHITECTURE_CONFLICT_AUDIT.md`) |
...
`entry_2_sweep`. Both pairs coexist by design (`ARCHITECTURE_CONFLICT_AUDIT.md`:
