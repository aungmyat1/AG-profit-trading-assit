"""AG_SSC_HYP_001_ROUTE_B_CORRECTED_CONTROL_3R (RB-C0/C1/C2).

Executes the preregistered corrected 3.0R CONTROL over the already-FROZEN Route B
Phase 1 occurrence population. This is the Phase 2 step of the Route B reconstruction
contract (RB-G4): outcome resolution ONLY, using the frozen v1.0.1 resolver
(session_sweep_continuation.outcome_resolution.resolve_campaign_entry) with
runner_target_r=3.0 and OPPOSITE_SESSION_BOUNDARY partial-target semantics.
...
STRICT FIREWALLS (enforced by construction, not by convention):
  ...
  * Only the CONTROL (runner_target_r=3.0) is executed. The 1.5R TREATMENT is NEVER
    computed.
  ...
  * Places no orders. Modifies no strategy/config/demo/live authority file.
...
OUT_DIR = (
    REPO_ROOT / "artifacts" / "validation" / "ST_SESSION_SWEEP_CONTINUATION_V1"
    / "HYP_001" / "ROUTE_B_CORRECTED_CONTROL_3R"
)
...
def main():
    ...
    if float(tm.get("runner_target_r")) != RUNNER_TARGET_R:
        raise SystemExit(f"RUNNER_TARGET_MISMATCH: {tm.get('runner_target_r')!r}")
    ...
    # per-setup economics (combined population) -- RB-C2 S1/S2/S3
    setup_metrics = {}
    ...
    control_outcome_hash = hashlib.sha256(outcomes_payload.encode("utf-8")).hexdigest()
    ...
    evidence = {
        ...
        "control_outcome_hash": control_outcome_hash,
    }
    control_evidence_hash = hashlib.sha256(
        json.dumps(evidence, sort_keys=True, separators=(",", ":"), default=str).encode("utf-8")
    ).hexdigest()

    OUT_DIR.mkdir(parents=True, exist_ok=True)
    (OUT_DIR / "CONTROL_OUTCOMES.json").write_text(
        json.dumps(all_outcomes, indent=2, sort_keys=True, default=str), encoding="utf-8",
    )
    (OUT_DIR / "CONTROL_SUMMARY.json").write_text(
        json.dumps({**evidence, "control_evidence_hash": control_evidence_hash},
