...
3. `OverallState.PARTIAL`/`INDETERMINATE` and `SIZING`'s `UNSIGNED_POLICY` are reserved
   ...
   `ENTRY_CONFIRMATION_V1_SPEC.md`'s `UNSIGNED_RULE` pattern).
