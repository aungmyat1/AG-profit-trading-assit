...
| 2 — Stage 1 exactly-once FX ticket delivery | **IMPLEMENTED, ARCHIVE-ONLY ACTIVATED AND VERIFIED WITH REAL SCHEDULED EVIDENCE; MESSAGE DELIVERY NOT AUTHORIZED** | `src/ticket_delivery/` (identity, archive, delivery journal, renderer, message-only Telegram transport, fx-cycle orchestration, scheduler call-site wiring, signed catch-up/retry policy) all implemented and tested; wired into `scripts/run_post_asian_pilot.py::_run_once()` under a `DISABLED`/`ARCHIVE_ONLY`/`MESSAGE_DELIVERY` mode contract. `config/ticket_delivery.yaml` now ships `mode: ARCHIVE_ONLY` with an OWNER_APPROVED policy block (60 min catch-up / 3 attempts / 30s-300s backoff, 2026-09-08 -- see `docs/status/AG_STAGE1_CATCHUP_AND_RETRY_POLICY_DECISION_PACKET_V1.md`). Real scheduled-task evidence confirms archive-only operation against live cycle output, including a genuine catch-up rejection of a naturally-occurring stale READY signal. `MESSAGE_DELIVERY` remains inert/unauthorized; WP7 (real Telegram send, delivered natural READY capture) not started. Full detail: `docs/plans/AG_STAGE1_EXACTLY_ONCE_FX_TICKET_DELIVERY_V1.md`. |
