...
def binding(strategy_id="TEST_STRATEGY"):
    return StrategyBinding(
        strategy_id=strategy_id,
        semantic_version="1.0.0",
        engine_id="existing_engine",
        engine_version="1",
...
class Adapter:
    ...
    def supports(self, event, binding):
        return True
    ...
    def project(self, observation):
        return FunnelProjection(
            ...
            reason_codes=("CANONICAL_SETUP",),
            setup_evidence={"setup_id": "setup-1"},
        )
...
def test_first_evaluation_is_deterministic():
    ...
    assert transition.to_stage == STAGE_SETUP_DETECTED
...
def test_existing_candidate_keeps_identity_and_increments_revision():
    ...
    second, transition = evaluate_funnel(
        event=event(), binding=binding(), adapter=Adapter(), previous_candidate=first
    )
    assert second.candidate_id == first.candidate_id
    assert second.occurrence_id == first.occurrence_id
    assert second.revision == 2
    assert transition.from_stage == first.stage
    assert transition.from_outcome == first.outcome
