"""TD-7: static guard proving src/mtf_context/topdown_composer.py never imports any
strategy-decision, proposal, risk, or execution module. Mirrors the AST-scan technique
already used by tests/test_shared_cache_no_strategy_import_guard.py and
...
def test_topdown_composer_only_imports_from_within_its_own_package_and_stdlib():
    ...
    for name in _imports_of(COMPOSER_PATH):
        ...
        assert is_relative_or_allowed or is_within_package, (
            f"{COMPOSER_PATH}: unexpected external import {name!r} -- composer should "
            "depend only on existing mtf_context context-producer interfaces"
        )
