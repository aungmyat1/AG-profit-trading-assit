...
def load_mt5_config() -> MT5Config:
    ...
    try:
        ...
        raise MT5ConfigError("CONFIG_ERROR: MT5_LOGIN is not a valid integer") from None

    password = os.environ.get("MT5_PASSWORD", "")
    if not password:
        raise MT5ConfigError("CONFIG_ERROR: MT5_PASSWORD is not set")

    server = os.environ.get("MT5_SERVER", "").strip()
    if not server:
        raise MT5ConfigError("CONFIG_ERROR: MT5_SERVER is not set")
